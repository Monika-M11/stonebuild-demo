...
export default function TransactionsTab({ projectName }: Props) {
    ...
    <FormProvider {...methods}>
      ...
      <div className="space-y-6">
        <div className="bg-white border rounded-lg p-6">
          <h3 className="text-lg font-semibold mb-4">Add Transaction - {projectName}</h3>
          <form onSubmit={methods.handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium mb-1 block">Date</label>
              <FormField type="datepicker" name="transaction_date" />
            </div>

            <div>
              <label className="text-sm font-medium mb-1 block">Description</label>
              <FormField type="input" name="description" placeholder="e.g. Cement purchase" />
            </div>

            <div>
              <label className="text-sm font-medium mb-1 block">Amount (₹)</label>
              <FormField type="input" name="amount" placeholder="0.00" className="numbers-decimal" />
            </div>
            ...
            <table className="table-default w-full">
              ...
              <tbody>
                {Array.from({ length: 32 }, (_, i) => ({
                  ...
                  description: ["Cement", "Steel", "Labour", "Paint", "Tiles", "Electrical"][i % 6] + " Payment",
                  amount: 12000 + (i * 3500),
                  type: i % 3 === 0 ? "credit" : "debit",
                  mode: ["NEFT", "UPI", "Cash", "Cheque"][i % 4],
                })).map((t) => (
                  <tr key={t.id}>
                    <td>{t.date}</td>
                    <td>{t.description}</td>
                    <td>
                      <span className={`px-3 py-1 rounded-full text-xs ${t.type === 'credit' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {t.type.toUpperCase()}
                      </span>
                    </td>
                    <td>{t.mode}</td>
