...
const dummyMaterials = Array.from({ length: 35 }, (_, i) => ({
  id: i + 1,
  material_name: ["Cement", "Steel Rod 12mm", "River Sand", "Red Brick", "Concrete Mix", "Ceramic Tiles", "Wall Paint", "Electrical Wire", "PVC Pipe", "Wooden Door"][i % 10],
  quantity: (50 + i * 5).toString(),
  unit: ["Bags", "Tons", "Cft", "Nos", "Liters", "Rolls"][i % 6],
  rate: (220 + i * 15).toString(),
}));

type Props = { projectId: string; projectName: string };

export default function MaterialsTab({ projectName }: Props) {
  const methods = useForm<FormValues>({
    defaultValues: { material_name: "", quantity: "", unit: "", rate: "" },
  });
    ...
    <FormProvider {...methods}>
      ...
      <div className="space-y-6">
        <div className="bg-white border rounded-lg p-6">
          <h3 className="text-lg font-semibold mb-4">Add Material - {projectName}</h3>
          <form onSubmit={methods.handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="text-sm font-medium mb-1 block">Material Name</label><FormField type="input" name="material_name" placeholder="Cement, Steel etc." /></div>
            <div><label className="text-sm font-medium mb-1 block">Quantity</label><FormField type="input" name="quantity" placeholder="Quantity" className="numbers-decimal" /></div>
            <div><label className="text-sm font-medium mb-1 block">Unit</label><FormField type="input" name="unit" placeholder="Bags / Tons / Nos" /></div>
            <div><label className="text-sm font-medium mb-1 block">Rate (₹)</label><FormField type="input" name="rate" placeholder="Rate per unit" className="numbers-decimal" /></div>

            <div className="md:col-span-2">
              <Button type="submit" disabled={loading} className="w-full">{loading ? "Adding..." : "Add Material"}</Button>
            </div>
          </form>
        </div>
              ...
              <tbody>
                {dummyMaterials.map((m) => (
                  <tr key={m.id}>
                    ...
                    <td>{m.quantity}</td>
                    <td>{m.unit}</td>
                    <td className="text-right">₹ {m.rate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
