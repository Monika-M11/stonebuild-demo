...
export default function QuotationList() {
  ...
  const fetchQuotations = useCallback(async () => {
    setLoading(true);
    try {
      const response = await postAPI("QUOTATION_LIST", {}, true); // Add this endpoint

      if (response.status === "success" && Array.isArray(response.data)) {
        setQuotationList(response.data);
        showToast("Quotation list loaded successfully");
      } else {
        showToast(response.message || "Failed to load quotations", "error");
      }
    } catch (error: any) {
      showToast(error.message || "Failed to fetch quotations", "error");
    } finally {
      setLoading(false);
    }
  }, []);
