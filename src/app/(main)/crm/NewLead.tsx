...
export default function LeadForm() {
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      const response = await postAPI("ADD_LEAD", payload, true); // ← Add this endpoint
      ...
      if (response.status === "success") {
        ...
        showToast(response.message || "Failed to add lead", "error");
      }
    } catch (err: any) {
  ...
  return (
    <FormProvider {...methods}>
      ...
      <div className="bg-white p-6">
        <form onSubmit={methods.handleSubmit(handleFormSubmit)} className="max-w-5xl">
          <div className="bg-gray-50 border rounded-xl p-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              {/* Customer Details */}
              <div>
                ...
                <div className="space-y-5">
                  ...
                  <Row label="Company Name">
                    <FormField type="input" name="company_name" placeholder="Enter Company Name" />
                  </Row>
                  ...
                  <Row label="Address">
                    <FormField type="textarea" name="address" placeholder="Enter Full Address" />
                  </Row>
          ...
          <div className="flex justify-end gap-4 mt-8">
            ...
            <Button variant="default" type="submit" disabled={loading} className="bg-[#103BB5] hover:bg-[#0f2e8a] px-8">
              {loading ? "Submitting..." : "Submit Lead"}
            </Button>
          </div>
        </form>
