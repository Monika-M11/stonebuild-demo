...
export default function LeadList() {
  ...
  const fetchLeads = useCallback(async () => {
    setLoading(true);
    try {
      const response = await postAPI("LEAD_LIST", {}, true); // Add this endpoint
              ...
              <tr key={item.id} className="border-b hover:bg-gray-50">
                ...
                <td className="text-center">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    item.lead_status === "Converted" ? "bg-green-100 text-green-700" :
                    item.lead_status === "Not Interested" ? "bg-red-100 text-red-700" :
                    item.lead_status === "Interested" ? "bg-blue-100 text-blue-700" : "bg-yellow-100 text-yellow-700"
                  }`}>
                    {item.lead_status}
                  </span>
                </td>
          ...
          <div className="fixed right-0 top-0 bottom-0 w-80 bg-white shadow-2xl z-50 overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="text-sm font-medium mb-2 block">Lead Status</label>
                  <select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="w-full border rounded-md px-3 py-2 text-sm">
                    <option value="">All Status</option>
                    <option value="New">New</option>
                    <option value="Contacted">Contacted</option>
                    <option value="Follow Up">Follow Up</option>
                ...
                <div>
                  <label className="text-sm font-medium mb-2 block">Lead Source</label>
                  <select value={selectedSource} onChange={(e) => setSelectedSource(e.target.value)} className="w-full border rounded-md px-3 py-2 text-sm">
                    <option value="">All Sources</option>
                    <option value="Website">Website</option>
                    <option value="Referral">Referral</option>
                    <option value="Call">Call</option>
