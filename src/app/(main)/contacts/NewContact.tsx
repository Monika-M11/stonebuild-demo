...
import { postAPI} from "@/app/utils/api"; // Adjust import path as needed
...
export default function ContactForm({ editId }: ContactFormProps) {
      ...
      try {
        ...
        if (response.status === "success" && response.data) {
          ...
          methods.reset({
            ...
            pan: contact.pan || "",
            registeration_id: contact.registeration_id || "",
            gst_no: contact.gst_no || "",
          });
        } else {
          showToast(response.message || "Failed to load contact details", "error");
        }
      } catch (err: any) {
  ...
  const confirmSubmit = async () => {
    ...
    try {
      const payload = {
        ...formData,
        ...(editId && { contact_id: editId }), // Send ID for update
      };

      const endpoint = editId ? "ADD_CONTACT" : "ADD_CONTACT"; // Same endpoint for now (common pattern)

      const response = await postAPI(endpoint, payload, true);

      if (response.status === "success") {
        showToast(
          editId ? "Contact updated successfully" : "Contact added successfully"
        );

        methods.reset();
        setTimeout(() => {
          router.push("/contacts"); // Change to your actual list route
        }, 1200);
      } else {
